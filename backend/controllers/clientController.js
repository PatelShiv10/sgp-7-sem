...
// @desc Auto-add client from existing user (NEW FUNCTION)
// @route POST /api/clients/auto-add/:userId
// @access Private (Lawyer)
const autoAddExistingClient = async (req, res) => {
  try {
    ...
    const clientRelation = await LawyerClient.create({
      ...
      status: 'active',
      caseType,
      caseTitle: caseTitle || `Case for ${user.firstName} ${user.lastName}`,
      caseDescription,
      notes: notes || `Client relationship created manually on ${new Date().toLocaleDateString()}`,
      preferredContact,
      hourlyRate,
      priority,
      lastContactDate: new Date(),
    ...
    console.error('Error auto-adding client:', error);
...
// @desc Search for users to add as clients (NEW FUNCTION)
// @route GET /api/clients/search-users
// @access Private (Lawyer)
const searchUsersToAdd = async (req, res) => {
  try {
    ...
    // Get existing client IDs to exclude them
    const existingClients = await LawyerClient.find({ lawyerId }).select('clientId');
    const existingClientIds = existingClients.map(client => client.clientId.toString());

    // Search for users (excluding lawyers and existing clients)
    ...
    const users = await User.find({
      $and: [
        ...
        { role: { $in: ['user'] } }, // Only regular users, not lawyers or admins
        { _id: { $nin: existingClientIds } } // Exclude existing clients
      ]
    })
...
// @desc Bulk operations on clients (NEW FUNCTION)
// @route POST /api/clients/bulk
// @access Private (Lawyer)
const bulkClientOperations = async (req, res) => {
  try {
    ...
    switch (action) {
      case 'archive':
        for (const clientId of clientIds) {
          try {
            await LawyerClient.findOneAndUpdate(
      ...
      case 'updateStatus':
        ...
        for (const clientId of clientIds) {
          try {
            await LawyerClient.findOneAndUpdate(
              { _id: clientId, lawyerId },
              { status: data.status, lastContactDate: new Date() }
            );
            results.success++;
          } catch (error) {
            results.failed++;
      ...
      case 'delete':
        for (const clientId of clientIds) {
          try {
            await LawyerClient.findOneAndDelete({ _id: clientId, lawyerId });
            results.success++;
          } catch (error) {
            results.failed++;
            results.errors.push({ clientId, error: error.message });
