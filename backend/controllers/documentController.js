...
=======
    // FIXED: Check if client exists as User
    ...
    // FIXED: Auto-create or verify client relationship
    let clientRelationship = await LawyerClient.findOne({
      lawyerId,
      clientId: clientId
    });

    if (!clientRelationship) {
      // Auto-create relationship when uploading documents
      try {
        clientRelationship = await LawyerClient.create({
          lawyerId,
          clientId,
          addedBy: 'manual',
          status: 'active',
          caseType: 'other',
          caseTitle: `Document-based case for ${clientUser.firstName} ${clientUser.lastName}`,
          notes: `Relationship created during document upload on ${new Date().toLocaleDateString()}`,
          lastContactDate: new Date(),
          caseStartDate: new Date()
        });
      } catch (relationshipError) {
        if (relationshipError.code === 11000) {
          // Duplicate key error - relationship exists, fetch it
          clientRelationship = await LawyerClient.findOne({
            lawyerId,
            clientId: clientId
          });
...
>>>>>>> 7c3075a18cacd5eb1f95050170ba624403a9fb14
    ...
    res.status(201).json({
      ...
      message: 'Document uploaded successfully',
      data: document,
      relationshipCreated: !clientRelationship.createdAt || 
        (new Date() - clientRelationship.createdAt) < 5000 // Created in last 5 seconds
    });
...
=======
    // FIXED: Verify client exists as a User first
    const clientUser = await User.findById(clientId);
    if (!clientUser) {
      return res.status(404).json({
        success: false,
    ...
    // FIXED: Check if LawyerClient relationship exists, create if missing
    let clientRelationship = await LawyerClient.findOne({
      lawyerId,
      clientId: clientId
    });

    // Auto-create relationship if documents exist but no relationship
    if (!clientRelationship) {
      ...
      if (hasDocuments > 0) {
        // Auto-create the relationship since documents exist
        try {
          clientRelationship = await LawyerClient.create({
            lawyerId,
            clientId,
            ...
            status: 'active',
            caseType: 'other',
            caseTitle: `Auto-generated case for ${clientUser.firstName} ${clientUser.lastName}`,
            notes: `Relationship auto-created due to existing documents on ${new Date().toLocaleDateString()}`,
            lastContactDate: new Date(),
            caseStartDate: new Date()
          });
        } catch (relationshipError) {
        ...
        return res.status(404).json({
          success: false,
          message: 'No client relationship found and no existing documents',
          suggestion: 'Please add this client to your client list first through the Client Management section',
          clientInfo: {
...
=======
          ...
          relationshipId: clientRelationship._id
>>>>>>> 7c3075a18cacd5eb1f95050170ba624403a9fb14
        }
      }
    });
...
// @desc Verify client relationship exists or create it
// @route POST /api/documents/verify-client/:clientId
// @access Private/Lawyer
const verifyOrCreateClientRelationship = async (req, res) => {
  try {
    const { clientId } = req.params;
    const lawyerId = req.user.id;
    ...
    if (!clientRelationship) {
      ...
      try {
        clientRelationship = await LawyerClient.create({
          ...
          status: 'active',
          caseType: 'other',
          caseTitle: `Case for ${clientUser.firstName} ${clientUser.lastName}`,
          notes: `Client relationship created for document management on ${new Date().toLocaleDateString()}`,
          lastContactDate: new Date(),
          caseStartDate: new Date()
        });
    ...
    res.status(500).json({
      success: false,
      message: 'Failed to verify or create client relationship'
    });
  }
};
...
=======
  getDocumentStats,
  verifyOrCreateClientRelationship
};
>>>>>>> 7c3075a18cacd5eb1f95050170ba624403a9fb14
