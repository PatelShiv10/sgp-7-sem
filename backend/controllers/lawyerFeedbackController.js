...
const createLawyerFeedback = async (req, res) => {
  try {
    ...
    const { lawyerId, rating, title, comment, serviceType = 'consultation' } = req.body;
    ...
    const lawyer = await User.findOne({ _id: lawyerId, role: 'lawyer', status: 'approved' });
    if (!lawyer) {
      return res.status(404).json({ success: false, message: 'Lawyer not found or not approved' });
    }
...
// @desc    Get all feedback for a specific lawyer
// @route   GET /api/lawyer-feedback/lawyer/:lawyerId
// @access  Public
const getLawyerFeedback = async (req, res) => {
  try {
    ...
    const isLawyerRequest = req.user && req.user.id === lawyerId;
    const query = { lawyerId };
    if (!isLawyerRequest) query.isApproved = true;
...
// @desc    Get feedback summary for a lawyer
// @route   GET /api/lawyer-feedback/lawyer/:lawyerId/summary
// @access  Public
const getLawyerFeedbackSummary = async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      data: { ...ratingStats, recentReviews }
    });
...
const respondToFeedback = async (req, res) => {
  try {
    ...
    const feedback = await LawyerFeedback.findOne({ _id: id, lawyerId, isApproved: true });
    if (!feedback) {
      return res.status(404).json({ success: false, message: 'Feedback not found or not accessible' });
    }
...
const markFeedbackHelpful = async (req, res) => {
  try {
    ...
    res.json({ success: true, data: { helpfulVotes: feedback.helpfulVotes } });
